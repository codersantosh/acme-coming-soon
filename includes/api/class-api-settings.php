 ...
 * @subpackage Acme_Coming_Soon/Acme_Coming_Soon_Api_Settings
 */
 ...
 *
 * @see Acme_Coming_Soon_Api
 */

if ( ! class_exists( 'Acme_Coming_Soon_Api_Settings' ) ) {

	/**
	 * Acme_Coming_Soon_Api_Settings
	 *
	 * @package Acme_Coming_Soon
	 * @since 1.0.0
	 */
	class Acme_Coming_Soon_Api_Settings extends Acme_Coming_Soon_Api {
		 ...
		 */
		public function run() {
			$this->type      = 'acme_coming_soon_api_settings';
		...
		public function get_item( $request ) {
			...
			$saved_options = acme_coming_soon_get_options();
		...
		protected function prepare_value( $value, $schema ) {
			...
			if ( isset( $sanitized_value['exclude']['ips'] ) ) {
				$sanitized_ips = array();
				foreach ( $sanitized_value['exclude']['ips'] as $key => $val ) {
					$sanitized_ip = acme_coming_soon_sanitize_ip( $val );
					if ( $sanitized_ip ) {
						$sanitized_ips[] = $sanitized_ip;
					}
				}
				$sanitized_value['exclude']['ips'] = $sanitized_ips;
			}
		...
		public function update_item( $request ) {
			...
			if ( is_wp_error( rest_validate_value_from_schema( $params, $schema ) ) ) {
				return new WP_Error(
					...
					sprintf( __( 'The %s property has an invalid stored value, and cannot be updated to null.', 'acme-coming-soon' ), ACME_COMING_SOON_OPTION_NAME ),
					array( 'status' => 500 )
				);
			}

			$sanitized_options = $this->prepare_value( $params, $schema );
			acme_coming_soon_update_options( $sanitized_options );
		...
		protected function get_registered_schema() {
			...
			$schema = acme_coming_soon_admin()->get_settings_schema();
...
/**
 * Return instance of  Acme_Coming_Soon_Api_Settings class
 *
 * @since 1.0.0
 *
 * @return Acme_Coming_Soon_Api_Settings
 */
function acme_coming_soon_api_settings() {
	return Acme_Coming_Soon_Api_Settings::get_instance();
}
acme_coming_soon_api_settings()->run();
